...
def policy_evaluation(P, R, policy, gamma=0.9, tol=1e-2):
    ...
    r_i = np.array([R[s,policy[s]] for s in range(Ns)])
    P_pi = np.zeros((Ns, Ns))
    for s in range(Ns):
        for s_next in range(Ns):
            P_pi[s, s_next]= P[s, policy[s], s_next]

    
    A = np.linalg.inv(np.eye(Ns) - gamma * P_pi)
    r_i = r_i[np.newaxis].T
    value_function=np.matmul(A, r_i)
...
def policy_iteration(P, R, gamma=0.9, tol=1e-3):
    ...
    new_policy = np.random.randint(Na, size=Ns)
    while not np.array_equal(new_policy,policy):
        policy = new_policy.copy()
        V = policy_evaluation(P, R, policy, gamma, tol)
        # greedy iteration
        for s in range(Ns):
            new_policy[s] = np.argmax([R[s,a] + gamma *sum([P[s,a,s_]*V[s_] for s_ in range(Ns)]) for a in range(Na)])
        # ====================================================
    return policy, V
...
def value_iteration(P, R, gamma=0.9, tol=1e-3):
    ...
    while np.max(np.abs(new_Q-Q)) > tol:
        ...
        for s in range(Ns):
            for a in range(Na):
                new_Q[s,a] = R[s, a] + (gamma* sum([P[s, a, s_]*max([Q[s_, a] for a in range(Na)]) for s_ in range(Ns)]))
        Qfs.append(new_Q)
